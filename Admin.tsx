        ...
        <div key={item.id} className="flex items-center justify-between bg-zinc-900/30 p-4 rounded-xl border border-white/5 hover:border-white/10 transition-all">
          ...
          <div className="flex gap-2 ml-4">
            ...
            <button 
              ...
              className="px-3 py-1 bg-red-900/20 text-red-500 text-[8px] font-bold rounded uppercase hover:bg-red-900/40 transition-colors"
            >
              Delete
            </button>
          </div>
...
const Admin: React.FC = () => {
  ...
  const handleDeleteProfileItem = async (category: string, id: string) => {
    if (window.confirm(t.confirmDelete)) {
      ...
      const collection = (currentProfile[key] as ProfileItem[]).filter(i => i.id !== id);
  ...
  const handleSaveEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingEvent) {
      // КЛЮЧЕВОЙ МОМЕНТ: если тип НЕ соревнование, удаляем данные о месте
              ...
              <div key={event.id} className="bg-zinc-900/40 border border-white/5 p-4 rounded-3xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0 text-left flex-1">
                  ...
                  <div className="truncate">
                    ...
                    <div className="text-[8px] text-zinc-500 uppercase mt-1">{event.date} • {event.type} {event.place ? `• ${event.place} Place` : ''}</div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            <div className="space-y-12">
              ...
              <ProfileListManager 
                ...
                onDelete={(id: string) => handleDeleteProfileItem('education', id)}
              />
              <ProfileListManager 
                category="skills" 
                ...
                onDelete={(id: string) => handleDeleteProfileItem('skills', id)}
              />
              <ProfileListManager 
                category="practice" 
                ...
                onDelete={(id: string) => handleDeleteProfileItem('practice', id)}
              />
            </div>
