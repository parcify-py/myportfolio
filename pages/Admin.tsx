...
// Компонент списка для образования, навыков и практики
const ProfileListManager = ({ category, items, lang, t, onEdit, onCreate }: any) => (
      ...
      {items.map((item: ProfileItem) => (
        <div key={item.id} className="flex items-center justify-between bg-zinc-900/30 p-4 rounded-xl border border-white/5 hover:border-white/10 transition-all">
          ...
          <div className="flex gap-2 ml-4">
            <button 
              onClick={() => onEdit(item)} 
              className="px-3 py-1 bg-zinc-800 text-[8px] font-bold rounded uppercase hover:bg-zinc-700 transition-colors"
            >
              Edit
            </button>
          </div>
...
const Admin: React.FC = () => {
  ...
  // Fix: storageService.getProfile() is async, initialize with INITIAL_PROFILE
  ...
  const [adminTab, setAdminTab] = useState<'events' | 'profile' | 'export'>('events');
  ...
  // Fix: refreshData must be async to await storageService methods
  ...
  // Fix: handleSaveEvent must be async
  const handleSaveEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingEvent) {
      await storageService.saveEvent(editingEvent);
      const evs = await storageService.getEvents();
      setEvents(evs);
  ...
  // Fix: handleSaveProfileItem must be async
  const handleSaveProfileItem = async (e: React.FormEvent) => {
    ...
    const collection = [...((currentProfile[key]) as ProfileItem[])];
    ...
    const finalProfile = { ...currentProfile, [key]: collection };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, target: 'event' | 'profile' | 'item') => {
    ...
    // Fix: reader callback should handle async operations correctly
    reader.onloadend = async () => {
      const result = reader.result as string;
      if (target === 'event' && editingEvent) {
        setEditingEvent({ ...editingEvent, images: [...(editingEvent.images || []), result] });
      } else if (target === 'profile') {
        const currentProfile = await storageService.getProfile();
        const updated = { ...currentProfile, photos: [result] };
        await storageService.saveProfile(updated);
        setProfile(updated);
      } else if (target === 'item' && editingProfileItem) {
        setEditingProfileItem({ 
          ...editingProfileItem, 
          item: { ...editingProfileItem.item, images: [...(editingProfileItem.item.images || []), result] }
        });
      }
  ...
  const startCreateProfileItem = (category: string) => {
    setEditingProfileItem({
      category,
      item: {
        title: { en: '', ru: '', cs: '' },
        subtitle: { en: '', ru: '', cs: '' },
        date: '',
        description: { en: '', ru: '', cs: '' },
        images: []
      }
    });
    setFormLang('en');
    setShowProfileItemForm(true);
  };
  ...
  const insertTag = (tag: string, ref: React.RefObject<HTMLTextAreaElement | null>, updateFn: (v: string) => void) => {
    ...
    if (tag === 'a') {
      ...
      if (!url) return;
      newContent = `<a href="${url}" target="_blank" class="text-yellow-500 underline">${selected || 'link text'}</a>`;
    } else {
      newContent = `<${tag}>${selected}</${tag}>`;
    }
    const newVal = before + newContent + after;
    updateFn(newVal);
    setTimeout(() => {
      el.focus();
      el.setSelectionRange(start + newContent.length, start + newContent.length);
    }, 0);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-20 space-y-10 md:space-y-20">
      <div className="flex border-b border-white/5 overflow-x-auto no-scrollbar">
        ...
        <button onClick={() => setAdminTab('export')} className={`px-6 md:px-8 py-4 text-[10px] md:text-xs font-bold uppercase tracking-widest border-b-2 transition-all whitespace-nowrap ${adminTab === 'export' ? 'border-yellow-500 text-white' : 'border-transparent text-zinc-500 hover:text-white'}`}>Export Data</button>
      </div>

      {adminTab === 'events' && (
        <section>
          <div className="flex justify-between items-center mb-12">
            <h1 className="text-2xl md:text-5xl font-serif font-bold tracking-tight">{t.manageEvents}</h1>
          ...
          <div className="grid gap-4">
            {events.map(event => (
              <div key={event.id} className="bg-zinc-900/40 border border-white/5 p-4 rounded-3xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0 text-left">
                  <img src={event.images[0] || 'https://picsum.photos/seed/placeholder/200/200'} className="w-12 h-12 rounded-xl object-cover bg-black" />
                  <div className="truncate">
                    <h3 className="font-serif font-bold truncate text-zinc-100">{event.title[lang] || event.title['en']}</h3>
                    <div className="text-[8px] text-zinc-500 uppercase mt-1">{event.date} • {event.type}</div>
                  </div>
                </div>
                <div className="flex gap-2">
            ...
            {events.length === 0 && <div className="py-20 text-center text-zinc-600 italic">No events found.</div>}
          </div>
        </section>
      )}
      ...
      {adminTab === 'profile' && (
        <section className="space-y-16 animate-in fade-in duration-500">
           <div className="flex justify-between items-center mb-8">
            <h1 className="text-2xl md:text-5xl font-serif font-bold tracking-tight">{t.profileSettings}</h1>
            <button onClick={handleLogout} className="px-6 py-3 border border-white/10 rounded-full text-[10px] uppercase tracking-widest text-zinc-400 hover:text-white transition-colors">Logout</button>
          </div>
            ...
            <div className="space-y-12">
              <div>
                ...
                <div className="flex gap-4 mb-6">
                  <div className="w-32 h-32 rounded-3xl bg-zinc-900 overflow-hidden relative group border border-white/10">
                    <img src={profile.photos[0]} className="w-full h-full object-cover" />
                    <button type="button" onClick={() => profilePhotoRef.current?.click()} className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[8px] font-bold uppercase transition-opacity">Change Photo</button>
                  </div>
                </div>
                ...
                <div className="space-y-4 text-left">
                  {(['en', 'ru', 'cs'] as Language[]).map(l => (
                    <input key={l} type="text" value={profile.about[l] || ''} onChange={e => { 
                      const p = { ...profile, about: { ...profile.about, [l]: e.target.value } }; 
                      setProfile(p); 
                      storageService.saveProfile(p); 
                    }} className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 focus:border-yellow-500 text-sm text-zinc-100" placeholder={`${t.aboutMe} (${l})`} />
                  ))}
                </div>
              </div>
              ...
              <ProfileListManager 
                ...
                lang={lang} 
                t={t} 
                onEdit={(item: ProfileItem) => { setEditingProfileItem({ category: 'education', item }); setShowProfileItemForm(true); }}
                onCreate={() => startCreateProfileItem('education')}
              />
              <ProfileListManager 
                category="skills" 
                ...
                lang={lang} 
                t={t} 
                onEdit={(item: ProfileItem) => { setEditingProfileItem({ category: 'skills', item }); setShowProfileItemForm(true); }}
                onCreate={() => startCreateProfileItem('skills')}
              />
              <ProfileListManager 
                category="practice" 
                items={profile.practice} 
                lang={lang} 
                t={t} 
                onEdit={(item: ProfileItem) => { setEditingProfileItem({ category: 'practice', item }); setShowProfileItemForm(true); }}
                onCreate={() => startCreateProfileItem('practice')}
              />
            </div>
            
            <div className="bg-zinc-900/20 p-8 rounded-3xl border border-white/5 h-fit lg:sticky lg:top-32">
              <h3 className="text-xs font-bold uppercase tracking-widest mb-6 text-yellow-500 text-left">Social Connections</h3>
              <div className="space-y-4">
                {socials.map(link => (
                  <div key={link.id} className="flex gap-2">
                    ...
                    <button onClick={() => { const updated = socials.filter(s => s.id !== link.id); setSocials([...updated]); storageService.saveSocialLinks(updated); window.dispatchEvent(new CustomEvent('socialUpdate')); }} className="text-red-500 text-[14px] font-bold hover:text-red-400 transition-colors">×</button>
                  </div>
                ))}
                <button onClick={() => { const newS = { id: Math.random().toString(36).substr(2, 9), platform: 'New Social', url: '' }; const updated = [...socials, newS]; setSocials([...updated]); storageService.saveSocialLinks(updated); window.dispatchEvent(new CustomEvent('socialUpdate')); }} className="w-full py-2 border border-dashed border-white/10 rounded-lg text-[10px] font-bold text-zinc-500 hover:text-white hover:border-white/30 transition-all">Add Link</button>
              </div>
            </div>
          </div>
        </section>
      ...
      {adminTab === 'export' && (
        <section className="animate-in fade-in duration-500 text-left space-y-6">
          <h1 className="text-2xl md:text-5xl font-serif font-bold tracking-tight">Export for Deployment</h1>
          <p className="text-zinc-400 text-sm max-w-2xl">
            Because this app uses LocalStorage, changes you make are only visible to you. 
            To make your events permanent for all visitors, copy the JSON below and replace 
            the initial values in <code className="text-yellow-500">constants.ts</code> before hosting.
          </p>
          <div className="relative group">
            <textarea 
              readOnly 
              value={getExportData()} 
              className="w-full h-96 bg-black border border-white/10 rounded-2xl p-6 font-mono text-[10px] text-zinc-400 focus:text-zinc-100 focus:border-yellow-500 transition-all no-scrollbar"
            />
            <button 
              onClick={() => { navigator.clipboard.writeText(getExportData()); alert('Copied to clipboard!'); }}
              className="absolute top-4 right-4 px-4 py-2 bg-yellow-500 text-black font-bold rounded-lg text-[10px] uppercase tracking-widest hover:bg-yellow-400 transition-all"
            >
              Copy JSON
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-[#0f0f0f] border border-white/10 rounded-3xl w-full max-w-2xl p-8 max-h-[90vh] overflow-y-auto relative no-scrollbar shadow-2xl">
            ...
            <div className="flex gap-2 mb-8 border-b border-white/5 pb-4">
              {(['en', 'ru', 'cs'] as Language[]).map(l => (
                <button key={l} onClick={() => setFormLang(l)} className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all ${formLang === l ? 'bg-yellow-500 text-black' : 'text-zinc-500 hover:text-white'}`}>{l}</button>
              ))}
            </div>
            <form onSubmit={handleSaveEvent} className="space-y-6 text-left">
                  ...
                  <div className="flex gap-4">
                    {[1, 2, 3].map(p => (
                      <button 
                        key={p} 
                        type="button" 
                        onClick={() => setEditingEvent({ ...editingEvent, place: p as AwardPlace })}
                        className={`flex-1 py-3 rounded-xl border font-bold text-xs transition-all ${editingEvent.place === p ? 'bg-yellow-500 border-yellow-500 text-black shadow-lg shadow-yellow-500/20' : 'bg-black border-white/10 text-zinc-500 hover:border-white/30'}`}
                      >
                        {p} Place
                      </button>
                    ))}
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold uppercase text-zinc-500">{t.description}</span>
                  <div className="flex gap-1">
                    <button type="button" onClick={() => insertTag('b', descRef, v => setEditingEvent({ ...editingEvent, description: { ...editingEvent.description!, [formLang]: v } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px] font-bold">B</button>
                    <button type="button" onClick={() => insertTag('i', descRef, v => setEditingEvent({ ...editingEvent, description: { ...editingEvent.description!, [formLang]: v } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px] font-italic">I</button>
                    <button type="button" onClick={() => insertTag('a', descRef, v => setEditingEvent({ ...editingEvent, description: { ...editingEvent.description!, [formLang]: v } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px]">URL</button>
                  </div>
                </div>
                ...
                <div className="flex gap-2 mb-4 flex-wrap">
                  {editingEvent.images?.map((img, idx) => (
                    <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border border-white/10 group">
                      ...
                      <button type="button" onClick={() => setEditingEvent({ ...editingEvent, images: editingEvent.images?.filter((_, i) => i !== idx) })} className="absolute inset-0 bg-red-600/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                    </div>
                  ))}
            ...
            <form onSubmit={handleSaveProfileItem} className="space-y-6 text-left">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold uppercase text-zinc-500">{t.description}</span>
                  <div className="flex gap-1">
                    <button type="button" onClick={() => insertTag('b', itemDescRef, v => setEditingProfileItem({ ...editingProfileItem, item: { ...editingProfileItem.item, description: { ...editingProfileItem.item.description!, [formLang]: v } } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px] font-bold">B</button>
                    <button type="button" onClick={() => insertTag('i', itemDescRef, v => setEditingProfileItem({ ...editingProfileItem, item: { ...editingProfileItem.item, description: { ...editingProfileItem.item.description!, [formLang]: v } } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px] font-italic">I</button>
                    <button type="button" onClick={() => insertTag('a', itemDescRef, v => setEditingProfileItem({ ...editingProfileItem, item: { ...editingProfileItem.item, description: { ...editingProfileItem.item.description!, [formLang]: v } } }))} className="px-2 py-1 bg-zinc-800 rounded text-[10px]">URL</button>
                  </div>
                </div>
                <textarea ref={itemDescRef} value={editingProfileItem.item.description?.[formLang] || ''} onChange={e => setEditingProfileItem({ ...editingProfileItem, item: { ...editingProfileItem.item, description: { ...editingProfileItem.item.description!, [formLang]: e.target.value } } })} className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 h-32 text-sm text-zinc-100" required />
              </div>
              <div>
                 <div className="flex gap-2 mb-4 flex-wrap">
                  {editingProfileItem.item.images?.map((img, idx) => (
                    <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border border-white/10 group">
                      <img src={img} className="w-full h-full object-cover" />
                      <button type="button" onClick={() => setEditingProfileItem({ ...editingProfileItem, item: { ...editingProfileItem.item, images: editingProfileItem.item.images?.filter((_, i) => i !== idx) } })} className="absolute inset-0 bg-red-600/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                    </div>
                  ))}
