...
const EventDetails: React.FC = () => {
  ...
  const handleDelete = async () => {
    if (event && window.confirm(t.confirmDelete)) {
  ...
  const awardConfig = (event.type === 'competition' && event.place) ? PLACE_CONFIG[event.place] : null;
  const title = event.title[lang] || event.title['en'];
  const description = event.description[lang] || event.description['en'];

  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-12 md:py-20 animate-in fade-in duration-500">
      ...
      <div className="space-y-8 md:space-y-12">
        <header className="space-y-4 md:space-y-6 text-left">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {event.type === 'journey' && (
              <span className="px-3 md:px-4 py-1 rounded-full border border-blue-500/30 text-blue-400 text-[9px] md:text-[10px] font-bold tracking-[0.2em] uppercase">
                {t.journey}
              </span>
            )}
          </div>
